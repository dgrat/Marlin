...
void inline incremental_WLSF(struct linear_fit_data *lsf, const float &x, const float &y, const float &z, const float &w) {
  ...
  lsf->yzbar += w * y * z;
  lsf->N     += w;
  lsf->max_absx = bigger(ABS(w * x), lsf->max_absx);
  lsf->max_absy = bigger(ABS(w * y), lsf->max_absy);
}
...
void inline incremental_LSF(struct linear_fit_data *lsf, const float &x, const float &y, const float &z) {
  ...
  lsf->xzbar += x * z;
  lsf->yzbar += y * z;
  lsf->max_absx = bigger(ABS(x), lsf->max_absx);
  lsf->max_absy = bigger(ABS(y), lsf->max_absy);
  lsf->N += 1.0;
}
