...
#include "../../../inc/MarlinConfig.h"
  ...
  void unified_bed_leveling::shift_mesh_height() {
    ...
    float unified_bed_leveling::measure_business_card_thickness(float in_height) {
      ...
      do_blocking_move_to(0.5f * (MESH_MAX_X - (MESH_MIN_X)), 0.5f * (MESH_MAX_Y - (MESH_MIN_Y)), in_height);
        //, smaller((planner.max_feedrate_mm_s[X_AXIS], planner.max_feedrate_mm_s[Y_AXIS]) * 0.5f);
      planner.synchronize();
  ...
  void unified_bed_leveling::smart_fill_mesh() {
    ...
    void unified_bed_leveling::tilt_mesh_based_on_probed_grid(const bool do_3_pt_leveling) {
      constexpr int16_t x_min = bigger(MIN_PROBE_X, MESH_MIN_X),
                        x_max = smaller(MAX_PROBE_X, MESH_MAX_X),
                        y_min = bigger(MIN_PROBE_Y, MESH_MIN_Y),
                        y_max = smaller(MAX_PROBE_Y, MESH_MAX_Y);
    ...
    void unified_bed_leveling::smart_fill_wlsf(const float &weight_factor) {
      ...
      const float weight_scaled = weight_factor * bigger(MESH_X_DIST, MESH_Y_DIST);
