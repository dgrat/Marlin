...
class unified_bed_leveling {
  ...
  public:
    ...
    static inline float z_correction_for_x_on_horizontal_mesh_line(const float &rx0, const int x1_i, const int yi) {
      ...
      return z1 + xratio * (z_values[smaller(x1_i, GRID_MAX_POINTS_X - 2) + 1][yi] - z1); // Don't allow x1_i+1 to be past the end of the array
    ...
    static inline float z_correction_for_y_on_vertical_mesh_line(const float &ry0, const int xi, const int y1_i) {
      ...
      return z1 + yratio * (z_values[xi][smaller(y1_i, GRID_MAX_POINTS_Y - 2) + 1] - z1); // Don't allow y1_i+1 to be past the end of the array
                                                                                      // If it is, it is clamped to the last element of the
                                                                                      // z_values[][] array and no correction is applied.
    }
    ...
    static float get_z_correction(const float &rx0, const float &ry0) {
      ...
      const float z1 = calc_z0(rx0,
                               ...
                               mesh_index_to_xpos(cx + 1), z_values[smaller(cx, GRID_MAX_POINTS_X - 2) + 1][cy]);

      const float z2 = calc_z0(rx0,
                               mesh_index_to_xpos(cx), z_values[cx][smaller(cy, GRID_MAX_POINTS_Y - 2) + 1],
                               mesh_index_to_xpos(cx + 1), z_values[smaller(cx, GRID_MAX_POINTS_X - 2) + 1][smaller(cy, GRID_MAX_POINTS_Y - 2) + 1]);
