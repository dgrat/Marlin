...
void homeaxis(const AxisEnum axis) {
  ...
  const float bump = axis_home_dir * (
    #if HOMING_Z_WITH_PROBE
      (axis == Z_AXIS && (Z_HOME_BUMP_MM)) ? bigger(Z_CLEARANCE_BETWEEN_PROBES, Z_HOME_BUMP_MM) :
    #endif
    home_bump_mm(axis)
  );
  ...
  void update_software_endstops(const AxisEnum axis) {
      ...
      if (axis == X_AXIS) {
        ...
        const float dual_max_x = bigger(hotend_offset[X_AXIS][1], X2_MAX_POS);
        ...
        if (active_extruder != 0) {
          ...
          soft_endstop_max[X_AXIS] = smaller(X1_MAX_POS, dual_max_x - duplicate_extruder_x_offset);
        }
        else {
      ...
      switch (axis) {
          ...
          case Y_AXIS:
            ...
            soft_endstop_radius = smaller(ABS(bigger(soft_endstop_min[X_AXIS], soft_endstop_min[Y_AXIS])), soft_endstop_max[X_AXIS], soft_endstop_max[Y_AXIS]);
