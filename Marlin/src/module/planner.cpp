...
void Planner::calculate_trapezoid_for_block(block_t* const block, const float &entry_factor, const float &exit_factor) {
  ...
  if (plateau_steps < 0) {
    ...
    accelerate_steps = smaller(uint32_t(bigger(accelerate_steps_float, 0)), block->step_event_count);
    plateau_steps = 0;
  ...
  if (current) {
    ...
    if (current->entry_speed_sqr != max_entry_speed_sqr || (next && TEST(next->flag, BLOCK_BIT_RECALCULATE))) {
      ...
      const float new_entry_speed_sqr = TEST(current->flag, BLOCK_BIT_NOMINAL_LENGTH)
        ? max_entry_speed_sqr
        : smaller(max_entry_speed_sqr, max_allowable_speed_sqr(-current->acceleration, next ? next->entry_speed_sqr : sq(float(MINIMUM_PLANNER_SPEED)), current->millimeters));
...
bool Planner::_populate_block(block_t * const block, bool split_move,
  ...
  block->step_event_count = bigger(block->steps[A_AXIS], block->steps[B_AXIS], block->steps[C_AXIS], esteps);
  ...
  // Max segment time in µs.
    ...
    const uint32_t max_x_segment_time = bigger(xs0, xs1, xs2),
                   max_y_segment_time = bigger(ys0, ys1, ys2),
                   min_xy_segment_time = smaller(max_x_segment_time, max_y_segment_time);
  ...
  float vmax_junction_sqr; // Initial limit on the segment entry velocity (mm/s)^2
    ...
    if (moves_queued && !UNEAR_ZERO(previous_nominal_speed_sqr)) {
      ...
      vmax_junction_sqr = smaller(vmax_junction_sqr, block->nominal_speed_sqr, previous_nominal_speed_sqr);
    }
    ...
    if (moves_queued && !UNEAR_ZERO(previous_nominal_speed_sqr)) {
      ...
      vmax_junction = smaller(nominal_speed, previous_nominal_speed);
      ...
      #endif
      {
        ...
        const float jerk = (v_exit > v_entry)
            ? //                                  coasting             axis reversal
              ( (v_entry > 0 || v_exit < 0) ? (v_exit - v_entry) : bigger(v_exit, -v_entry) )
    ...
    #if ENABLED(JUNCTION_DEVIATION)
      vmax_junction_sqr = smaller(vmax_junction_sqr, sq(vmax_junction));
    #else
  ...
  block->entry_speed_sqr = !split_move ? sq(float(MINIMUM_PLANNER_SPEED)) : smaller(vmax_junction_sqr, v_allowable_sqr);
