...
uint16_t max_display_update_time = 0;
  ...
  void scroll_screen(const uint8_t limit, const bool is_menu) {
    ...
    if (screen_items > 0 && encoderLine >= screen_items - limit) {
      encoderLine = bigger(0, screen_items - limit);
...
void lcd_quick_feedback(const bool clear_buttons) {
   ...
   */
    ...
    static void IDEX_menu() {
      ...
      MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float52, MSG_IDEX_X_OFFSET , &hotend_offset[X_AXIS][1], smaller(X2_HOME_POS, X2_MAX_POS) - 25.0, bigger(X2_HOME_POS, X2_MAX_POS) + 25.0, _recalc_IDEX_settings);
  ...
  void _lcd_preheat(const int16_t endnum, const int16_t temph, const int16_t tempb, const int16_t fan) {
    if (temph > 0) thermalManager.setTargetHotend(smaller(heater_maxtemp[endnum], temph), endnum);
    #if HAS_HEATED_BED
  ...
  float move_menu_scale;
    ...
    void _man_probe_pt(const float &rx, const float &ry) {
      ...
      lcd_synchronize();
      move_menu_scale = bigger(PROBE_MANUALLY_STEP, MIN_STEPS_PER_SEGMENT / float(DEFAULT_XYZ_STEPS_PER_UNIT));
  ...
  void lcd_advanced_temperature_menu() {
    ...
    void _lcd_configuration_temperature_preheat_settings_menu(const uint8_t material) {
      #if HOTENDS > 5
        #define MINTEMP_ALL smaller(HEATER_0_MINTEMP, HEATER_1_MINTEMP, HEATER_2_MINTEMP, HEATER_3_MINTEMP, HEATER_4_MINTEMP, HEATER_5_MINTEMP)
        #define MAXTEMP_ALL bigger(HEATER_0_MAXTEMP, HEATER_1_MAXTEMP, HEATER_2_MAXTEMP, HEATER_3_MAXTEMP, HEATER_4_MAXTEMP, HEATER_5_MAXTEMP)
      #elif HOTENDS > 4
        #define MINTEMP_ALL smaller(HEATER_0_MINTEMP, HEATER_1_MINTEMP, HEATER_2_MINTEMP, HEATER_3_MINTEMP, HEATER_4_MINTEMP)
        #define MAXTEMP_ALL bigger(HEATER_0_MAXTEMP, HEATER_1_MAXTEMP, HEATER_2_MAXTEMP, HEATER_3_MAXTEMP, HEATER_4_MAXTEMP)
      #elif HOTENDS > 3
        #define MINTEMP_ALL smaller(HEATER_0_MINTEMP, HEATER_1_MINTEMP, HEATER_2_MINTEMP, HEATER_3_MINTEMP)
        #define MAXTEMP_ALL bigger(HEATER_0_MAXTEMP, HEATER_1_MAXTEMP, HEATER_2_MAXTEMP, HEATER_3_MAXTEMP)
      #elif HOTENDS > 2
        #define MINTEMP_ALL smaller(HEATER_0_MINTEMP, HEATER_1_MINTEMP, HEATER_2_MINTEMP)
        #define MAXTEMP_ALL bigger(HEATER_0_MAXTEMP, HEATER_1_MAXTEMP, HEATER_2_MAXTEMP)
      #elif HOTENDS > 1
        #define MINTEMP_ALL smaller(HEATER_0_MINTEMP, HEATER_1_MINTEMP)
        #define MAXTEMP_ALL bigger(HEATER_0_MAXTEMP, HEATER_1_MAXTEMP)
      #else
