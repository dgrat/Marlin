      ...
      if (theCard.longFilename[0]) {
          ...
          if (sel) {
            ...
            if (filename_scroll_hash != name_hash) {                            // If the hash changed...
              ...
              filename_scroll_max = bigger(0, utf8_strlen(theCard.longFilename) - n);  // Update the scroll limit
  ...
  #define lcd_implementation_drawmenu_function(sel, row, pstr, data) lcd_implementation_drawmenu_generic(sel, row, pstr, '>', ' ')
    ...
    */
    ...
    void lcd_implementation_ubl_plot(const uint8_t x, const uint8_t inverted_y) {
        ...
        clear_custom_char(&new_char);
        const uint8_t ypix = smaller(upper_left.y_pixel_offset + pixels_per_y_mesh_pnt, ULTRA_Y_PIXELS_PER_CHAR);
